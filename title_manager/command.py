...
def register(server: PluginServerInterface, manager: TitleManager) -> None:
    ...
    server.register_command(
        ...
        .then(Literal('set')
            .then(Text('id').runs(lambda src, ctx: manager.cmd_set(src, ctx['id'])))
        )
                    ...
                    .then(Text('color')
                        .then(Text('bold').runs(
                            lambda src, ctx: manager.cmd_add(
                                src, ctx['id'], ctx['name'], ctx['color'], ctx['bold']
                            )
                        ))
                    )
                )
        ...
        .then(Literal('remove').requires(is_admin)
            .then(Text('id').runs(lambda src, ctx: manager.cmd_remove(src, ctx['id'])))
        )
        .then(Literal('join').requires(is_admin)
            .then(Text('player')
                .then(Text('id').runs(lambda src, ctx: manager.cmd_join(src, ctx['player'], ctx['id'])))
            )
        )
        .then(Literal('give').requires(is_admin)
            .then(Text('player')
                .then(Text('id').runs(lambda src, ctx: manager.cmd_give(src, ctx['player'], ctx['id'])))
            )
        )
        .then(Literal('delete').requires(is_admin)
            .then(Text('player')
                .then(Text('id').runs(lambda src, ctx: manager.cmd_delete(src, ctx['player'], ctx['id'])))
            )
        )
